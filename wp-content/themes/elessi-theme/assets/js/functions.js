...
function hide_compare($) {
    if ($('.nasa-compare-list-bottom').length) {
        $('.transparent-window').fadeOut(500);
...
function init_shipping_free_notification($, confetti) {
    if ($('.nasa-total-condition').length) {
        ...
        if ($('.nasa-after-add-to-cart-popup').length && $('.nasa-after-add-to-cart-popup').parents('.mfp-container').find('#nasa-confetti').length <=0) {
            if($('#cart-sidebar').find('#nasa-confetti').length) {
                $('.nasa-after-add-to-cart-popup').parents('.mfp-container').append('<canvas id="nasa-confetti" style="display: none;">');
            }
        }
